...
export class RoutingPageComponent implements OnInit {
  ...
  routingSubmit(): void {
    if (this.routingForm.value.departure === this.routingForm.value.destination) {
      // "Please choose departure and destination correctly";
      console.log("Please choose departure and destination correctly");
      return
    }
    this.fare = this.fareCalculator();

    //store partial to db
    this.bookingService.createPlaceholder({...this.routingForm.value, fare: this.fare, booked_by: this.username}).subscribe(data => {
      if(data){
  ...
  fareCalculator(): number {
    ...
    return this.fare;
    // this.bookingService
    //   .createPlaceholder({ ...this.routingForm.value, fare: this.fare })
    //   .subscribe(() => this.router.navigate(['/accomodation']));
  }
}
